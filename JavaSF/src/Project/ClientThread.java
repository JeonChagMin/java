...
public class ClientThread extends Thread {
	...
	private ChatRoomDisplay chatRoom;
	...
	public static String ct_logonID;
//	다른 곳에서 LogonId 값을 접근 할 수 있게 private에서 pubulic으로 교체
...
//		내컴퓨터에서 연결
		...
		ct_chatRoom = null;
		try {
			ct_sock = new Socket(InetAddress.getLocalHost(), 7777);
...
//		다른컴퓨터를 연결하기 위해서 다른컴퓨터의 ip주소를 받아옴
		ct_waitRoom = new WaitRoomDisplay(this);
		ct_chatRoom = null;
		try {
			ct_sock = new Socket(hostaddr, 7777);
			ct_in = new DataInputStream(ct_sock.getInputStream());
			ct_out = new DataOutputStream(ct_sock.getOutputStream());
			ct_buffer = new StringBuffer(4096);
			thisThread = this;
	...
	public void run() {
//		ChatClient에서 thread.start()를 호출하면 실행되는 메소드
		try {
			Thread currThread = Thread.currentThread();
			while (currThread == thisThread) {
				String recvData = ct_in.readUTF();
				StringTokenizer st = new StringTokenizer(recvData, SEPARATOR);
//				recvData에서의 문자열을 SEPARATOR="|"을 기준으로 분할하기 위한 기능
...
//						로그인한 유저를 Vector에 추가
				...
				case YES_CREATEROOM: {
					...
					if (ct_chatRoom == null) {
						...
						ct_chatRoom.coerceOut.setEnabled(true);
					} else {
						ct_chatRoom.show();
						ct_chatRoom.isAdmin = true;
...
//					st1은 방을 나눔 	st2는 유저
				...
				case YES_QUITROOM: {
					...
					if (ct_chatRoom.isAdmin) {
						ct_chatRoom.coerceOut.setEnabled(false);
						ct_chatRoom.isAdmin = false;
					}
					
//					if(user.get(1)==chat.userVector.get(1)) {
//						ct_chatRoom.isAdmin=true;
//						ct_chatRoom.coerceOut.setEnabled(true);
//					}
